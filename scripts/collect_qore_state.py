...
def main() -> int:
    ...
    snapshot = {
        ...
        "main_sha": main_sha,
        "tree_sha": tree_sha,
        "main_protected_expected": True,
        "quality_gate_required_expected": "quality",
        ...
        "mission_document_paths": [
            p.relative_to(root).as_posix() for p in sorted(root.glob("docs/missions/**/*.md")) if p.is_file()
        ],
        "architecture_document_paths": [
            p.relative_to(root).as_posix() for p in sorted(root.glob("docs/architecture/**/*.md")) if p.is_file()
        ],
    ...
    print(f"QORE state snapshot: main={main_sha} tree={tree_sha}")
    print(f"roadmaps={len(snapshot['roadmap_documents'])} open_prs={len(pulls)} open_issues={len(issues)} errors={len(errors)}")
    return 0
