...
- The checked-out QORE repository must match that SHA before engineering begins.
...
- Inspect the relevant code, tests, contracts and documentation before proposing modifications.
...
- Preserve the repository's quality gate and architecture invariants.
...
In the initial orchestrator rollout, operate in PLAN-ONLY mode. Produce an implementation plan bound to the exact source SHA; do not mutate `qore-core`. A later explicitly reviewed orchestrator stage may grant bounded branch/PR write authority.
